...
dataTrain_matrix <- Matrix::sparse.model.matrix(Species~ .-1, data = data_train)
output_vector_train = ifelse( data_train[,'Species'] == "setosa",1,data_train[,'Species'] )-1
train_matrix <- xgb.DMatrix(data = as.matrix(dataTrain_matrix),label=output_vector_train)
dataTest_matrix <- Matrix::sparse.model.matrix(Species ~ .-1, data = data_test)
output_vector_test = ifelse( data_test[,'Species'] == "setosa",1,data_test[,'Species'] )-1
...
which.min(evalue_log$test_mlogloss)
# 依照最佳迭代次數再次建模
bst_model <- xgb.train(params = params,
                       data = train_matrix,
                       nrounds = which.min(evalue_log$test_mlogloss),
                       watchlist = watchlist,
                       eta = 0.3,     # Learning Rate, low -> more robust to overfitting
                       max.depth = 5, #預設值:6，每顆樹的最大深度，樹高越深，越容易overfitting
                       seed =123
